...
async def on_message(message: discord.Message) -> None:
    ...
    if message.channel.id in db.get_ban_channel_by_guild_id(message.guild.id):
        ...
        await bot.get_channel(int(os.getenv('LOGS_CHANNEL'))).send(embed=message_maker.ban_embed(str(message.author), message.content, bot.get_channel(int(os.getenv('BOT_BAIT_CHANNEL')))))
        return
